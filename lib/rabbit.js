/**
 * Created by L on 2015/12/14.
 */
var amqp = require('amqp');
var logger = require('./logger').logger('rabbit');

var addresses=[];

if(config.rabbit_store.addresses){
    var addreGroups=config.rabbit_store.addresses.split(",");
    for(var i=0;i<addreGroups.length;i++){
        var addres=addreGroups[i].split(":");
        if(addres.length ==2){
            addresses.push({host:addres[0],port:addres[1]});
        }
    }
}

function getAddre(excludeHost){
    if(addresses.length ==0){
        throw "not find rabbit host";
    }
    for(var i=0;i<addresses.length;i++){
        var addre=addresses[i];
        if(excludeHost){
            if(addre.host == excludeHost){
                continue;
            }
        }
        addre.login=config.rabbit_store.username;
        addre.password=config.rabbit_store.password;
        return addre;
    }
}

exports.connection=function (){
    if(addresses.length ==0){
        throw "not find rabbit host";
    }

    var connection =amqp.createConnection(getAddre(),{reconnect: false,defaultExchangeName: 'amq.topic'});
    connection.on('error', function (error) {
        //如果有异常尝试连接其它机器
        logger.error('Rabbit Connection error' + error+';host:'+connection.options.host);
        connection.setOptions(getAddre(connection.options.host));
        connection.reconnect();
        logger.info('Rabbit Attempts to reconnect');
    });
    connection.on('close', function () {
        logger.info('Rabbit Connection close');
    });
    connection.on('connect', function () {
        logger.info('Rabbit Connection connect Success');
    });
    if(connection){
        return connection;
    }
};