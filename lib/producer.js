...
function producer(route,message,option){
    ...
    self.connection.on('ready', function () {
        ...
        self.connection.exchange('exchange_global',_.extend((option||{}),{type: 'topic',autoDelete:false,durable:true,confirm:true}),function(exchange){
            logger.info('Producer Exchange ' + exchange.name + ' is open');
            exchange.publish(route,message,{immediate:true,timestamp:new Date()},function(state,err) {
