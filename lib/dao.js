...
MessageDao.prototype.getUserMessages = function (socket, callback) {
	var self = this;
	var deferred = Q.defer();
	var userEmail = socket.UserEmail;
	var companyId = socket.CompanyId;
	self.getAll().then(function (messages) {
		var userMessages = messages.filter(function (message) {
			if (_.contains(message.UserEmail, userEmail) || _.contains(message.CompanyId, companyId) || message.AllUser) {
				return true;
			}
		});
